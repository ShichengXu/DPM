		...
		if (locType == LocalizationType.FALLING_EDGE) {              //falling edge
			...
			while(moveX==true)
			{
				...
				if (touchSample1[0]==1 && touchSample2[0]==1)
				{
					leftMotor.stop(true);
					rightMotor.stop(false);
					odo.setPosition(new double[] { -10.0, 0.0, 0.0 }, new boolean[] {true, false, false});
					break;
				}
				
			}
			
			leftMotor.rotate(convertDistance(2.1,-10),true);
			rightMotor.rotate(convertDistance(2.1,-10),false);
			turnAngle(90);
			...
			while(moveY==true)
			{
				...
				if (touchSample1[0]==1 && touchSample2[0]==1)
				{
					leftMotor.stop(true);
					rightMotor.stop(false);
					odo.setPosition(new double[] { 0.0, -10.0, 270.0 }, new boolean[] {false, true, true});
					break;
				}
