...
public class Localization {
	...
	public Localization(Odometer odo, SampleProvider usSensor, float[] usData, SampleProvider touch1, SampleProvider touch2,float[] touchSample1,float[] touchSample2,
			...
			EV3LargeRegulatedMotor rightMotor,EV3LargeRegulatedMotor sensorMotor) {
		this.odo = odo;
	...
	public void doLocalization() {
		...
		if (locType == LocalizationType.FALLING_EDGE) {              //falling edge
			...
			odo.setPosition(new double[] { 0.0, 0.0, odoUpdate1[0] }, new boolean[] {   //update the odometer to 0,0,90 (to make our second part easier )
					true, true, true });
			...
			while(moveX==true)
			{
				...
				if (touchSample1[0]==1 && touchSample2[0]==1)
				{
					...
					if(FinalProject.startingCorner==1 ||FinalProject.startingCorner==3)
					{
					odo.setPosition(new double[] { odoUpdate2[0], 0.0, 0.0 }, new boolean[] {true, false, false});
					}
					else
					{
					odo.setPosition(new double[] { 0, odoUpdate2[1], 0.0 }, new boolean[] {false, true, false});
					}
					break;
				}
			...
			while(moveY==true)
			{
				...
				if (touchSample1[0]==1 && touchSample2[0]==1)
				{
					...
					odo.setPosition(new double[] { 0.0, odoUpdate2[1], odoUpdate1[1] }, new boolean[] {false, true, true});
					
					}
					else
					{

						odo.setPosition(new double[] { odoUpdate2[0], 0, odoUpdate1[1] }, new boolean[] {true, false, true});
   ...
   private static double[] odoTheta(int targetCorner) {
		...
		return result;
		//targetXY=coordinateHelper(corner,dropZone_X,dropZone_Y);
	}
